  ...
  methods: {
    addMediaItem() {
      ...
      const src =
        ...
        self.computedSerie.poster_path;

      let data = {
        id: self.computedSerie.id,
        title: self.computedSerie.name,
        src: src,
        type: "tv",
        duration: self.computedSerieDetails.episode_run_time[0],
        releaseDate: self.computedSerieDetails.first_air_date
      };
    ...
    goToDetail() {
      ...
      this.$router.push({
        name: "detail",
        params: { detailId: self.computedSerie.id, mediaType: "tv" }
      });
    },
    handleClickAway() {
      this.isActive = true;
