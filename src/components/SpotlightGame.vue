...
export default {
  ...
  methods: {
    addMediaItem() {
      ...
      const src =
        "https://images.igdb.com/igdb/image/upload/t_720p/" +
        self.computedGame.cover +
        ".jpg";

      let data = {
        id: self.computedGame.id,
        title: self.computedGame.name,
        src: src,
        type: "game",
        duration: 0,
        releaseDate: self.computedReleaseYear
      };
    ...
    goToDetail() {
      ...
      this.$router.push({
        name: "detail",
        params: { detailId: self.computedGame.id, mediaType: "game" }
      });
    },
    handleClickAway() {
      this.isActive = true;
    ...
    addToSeen() {
      ...
      const props = { id: self.computedGame.id, timestamp: today };
